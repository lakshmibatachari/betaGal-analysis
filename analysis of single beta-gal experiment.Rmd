...
Add seven more columns immediately to the right: experimenter, date, strain, RpoS, dilutionFactor, volumeAssayed, reactionTime, drop, and dropExplanation. (note the capitalization and spaces)
...
8. For **drop**, enter TRUE to drop the data and FALSE to keep the data in the analysis.
9. For **dropExplanation**, if drop == TRUE, give a brief reason for why the point should be dropped.  
...
```{r}
fileName <- "CRDdata/CRD 8-1-18 Strain 37 38 39 40.xlsx" #Enter your Excel file here. Remember that tab completion will help you get the path & name correct
...
#includes a step to filter out blanks and "dropped" wells
combinedData <-left_join(OD420data, meanCellDensities, by=c("strain", "RpoS")) %>%
                select(`Blank 420`, experimenter, date, strain, RpoS, dilutionFactor, volumeAssayed, reactionTime, meanDensity, drop) %>%
                filter(strain != "blank" & drop == "FALSE") %>% 
...
ggplot(combinedData) + geom_point(aes(x=RpoS, y=`Miller Units`)) +
